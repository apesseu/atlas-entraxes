"""
normalize_dept_zones.py

Script de normalisation des données départementales depuis Excel vers CSV.
Transforme le fichier dept_zones.xlsx en format normalisé pour l'application Atlas Entraxes.

Ce script lit un fichier Excel contenant les zones vent/neige par département et le convertit
en format CSV standardisé utilisable par l'application principale. Il effectue également
le nettoyage et la normalisation des données.

Usage:
    python scripts/normalize_dept_zones.py
...
if __name__ == "__main__":
    """
    Point d'entrée du script de normalisation.
    
    Exécute la normalisation et gère les codes de sortie système :
    - 0 : Succès
    - 1 : Erreur générique  
